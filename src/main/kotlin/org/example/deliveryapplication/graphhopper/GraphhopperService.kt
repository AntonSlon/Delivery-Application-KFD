package org.example.deliveryapplication.graphhopper

import org.example.deliveryapplication.database.entity.Courier
import org.example.deliveryapplication.database.entity.Order
import org.example.deliveryapplication.database.repository.CourierDAO
import org.example.deliveryapplication.database.repository.OrderDAO
import org.example.deliveryapplication.database.repository.UserDAO
import org.example.deliveryapplication.model.OrderStatus
import org.example.deliveryapplication.model.Role
import org.example.deliveryapplication.model.courier.Vehicle
import org.example.deliveryapplication.model.request.CustomerCreateOrderRequest
import org.example.deliveryapplication.model.response.RouteResponse
import org.example.deliveryapplication.util.getPrincipal
import org.slf4j.LoggerFactory
import org.springframework.stereotype.Service
import java.util.*

@Service
class GraphhopperService(
    val graphhopperClient: GraphhopperClient,
    val nominatimService: NominatimService,
    val orderDAO: OrderDAO,
    val courierDAO: CourierDAO,
    val userDAO: UserDAO,
) {


    fun getRoute(
        addresses: List<String>,
        profile: String,
        locale: String,
        calcPoints: Boolean
    ): RouteResponse {
        println("GEOCODE ADDRESS: $addresses")
        val jsonPoints = addresses.map { address -> nominatimService.geocodeAddress(address) }
        println("JSON POINTS: $jsonPoints")
        val points = jsonPoints.map { obj -> obj[0].lat + "," + obj[0].lon }
        println("POINTS: $points")
        val route = graphhopperClient.getRoute(points, profile, locale, calcPoints)

        val time = route.paths[0].time

//        if (userDAO.findById(getPrincipal().id).get().role == Role.COURIER.name) {
//
//            val courier: Courier = courierDAO.findById(getPrincipal().id).get()
//
//            if (time > 60_000)
//                throw Exception("Курьер слишком далеко")
//        }

        return graphhopperClient.getRoute(points, profile, locale, calcPoints)
    }

    //оч колхозное вычисление, в будующем можно улучшить логику рассчета
    fun calculatePrice(customerCreateOrderRequest: CustomerCreateOrderRequest): Int{
        val calc1 = getRoute(listOf(customerCreateOrderRequest.startAddress, customerCreateOrderRequest.endAddress), "foot", "de", false).paths[0].distance
        val calc2 = getRoute(listOf(customerCreateOrderRequest.startAddress, customerCreateOrderRequest.endAddress), "car", "de", false).paths[0].distance
        return ((((calc1 + calc2) / 2) / 1000) * 5).toInt()
    }
}